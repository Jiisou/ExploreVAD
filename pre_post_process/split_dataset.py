...
def split_dataset(root_dir: str, output_dir: str, extension: str, seed: int = 42):
    """
    ...
    for event_dir in event_dirs:
        ...
        print(f"Completed: {event_name:<5} (Total: {n_total} | Tr: {len(file_mapping['train'])}, Val: {len(file_mapping['valid'])}, Te: {len(file_mapping['test'])})")

if __name__ == "__main__":
    input_root = input("root_dir 경로를 입력하세요: ")
    output_root = input("output_dir 경로를 입력하세요: ")
    extension = input("탐색할 파일의 확장자 (e.g., mp4, npy): ")
    split_dataset(input_root, output_root, extension)