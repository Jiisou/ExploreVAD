#!/usr/bin/env python3
"""
Inference script for CLIP-based Anomaly Action Spotting Model.

Features:
- Process untrimmed videos with sliding window
- Output anomaly scores per segment
- Single video or batch processing
- Export results to CSV
- Optional visualization of anomaly timeline
"""
...
def process_video(
    ...
    Returns:
        List of dicts with segment info and anomaly scores.
    """
...
def visualize_results(
    ...
    if np.any(high_score_mask):
        for t, s, is_high in zip(timestamps, scores, high_score_mask):
            if is_high:
                plt.axvspan(t, t + 2.0, alpha=0.2, color='red')
...
def print_summary(results: List[Dict], video_path: str, threshold: float = 0.5):
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with CLIP-based Anomaly Action Spotting Model"
    )
    ...
    parser.add_argument("--overlap-ratio", type=float, default=0.0,
                        help="Window overlap ratio (0.0 = no overlap)")
    parser.add_argument("--unit-duration", type=float, default=2.0,
                        help="Unit duration in seconds")
