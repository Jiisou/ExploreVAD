...
Architecture:
    (B, T, D) → Temporal Agg → (B, D)
    → x + MLP(x)  [residual, QuickGELU]
    → Linear(D, 1) → BCEWithLogitsLoss

Key differences from train_linear.py (FeatureSpottingModel):
- Single logit output (not 2-class) with BCEWithLogitsLoss
- Residual MLP block with CLIP's QuickGELU activation
- pos_weight for class imbalance (instead of CrossEntropyLoss weight)
"""
...
def train_one_epoch(
    ...
    for inputs, labels in pbar:
        ...
        predicted = (logits > 0).long()  # threshold at 0 (logit space)
        total += labels.size(0)
...
def compute_pos_weight(dataset: NPYFeatureDataset) -> torch.Tensor:
    ...
    This upweights the positive (anomaly) class when it's underrepresented.
    """
...
def train(
    ...
    # Training loop
    print("\n" + "=" * 60)
    print("Training ResidualMLPSpottingModel")
    print("=" * 60)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train ResidualMLPSpottingModel on pre-extracted npy features (ETRI)"
    )
...
def main():
    ...
    print("=" * 60)
    print("ResidualMLP Anomaly Action Spotting - Training")
    print("=" * 60)
