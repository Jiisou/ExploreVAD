...
#TODO: gt txt에 없는 비디오만 CSV 에서 찾도록
...
def load_gt_from_temporal_annotation_txt(
    txt_path: str,
    video_filename: str,
    video_duration_sec: int = 0,
    annotation_fps: float = 30.0,
) -> Dict[int, int]:
    """Load GT annotation from Temporal_Anomaly_Annotation.txt for a specific video.

    Row format (space-separated):
        video_name  event_type  start1  end1  start2  end2

    Frames are at annotation_fps (default 30). -1 means no event instance.
    Normal videos have -1 in all four frame columns.
    ...
    anomaly_segments: List[Tuple[int, int]] = []  # (start_sec, end_sec)
    is_normal_video = False
    found = False
    ...
    with open(txt_path, 'r', encoding='utf-8-sig') as f:
        for line in f:
            ...
            fname = parts[0]
            fname_stem = os.path.splitext(fname)[0]
            if fname != video_filename and fname_stem != video_stem:
                continue

            found = True
            event_type = parts[1].lower()
            is_normal_video = (event_type == 'normal')
            ...
            if s1 >= 0 and e1 >= 0:
                anomaly_segments.append((
                    int(s1 / annotation_fps),
                    int(e1 / annotation_fps),
                ))
            # Second instance (optional)
            if s2 >= 0 and e2 >= 0:
                anomaly_segments.append((
                    int(s2 / annotation_fps),
                    int(e2 / annotation_fps),
                ))
            break
...
def process_video_realtime(
    ...
    # Load GT annotation (txt or csv)
    if gt_timestamps_info:
        txt_path   = gt_timestamps_info.get('txt_path', '')
        ...
        if txt_path:
            # --- Temporal_Anomaly_Annotation.txt (frame-based, 30 fps) ---
            gt_anno = load_gt_from_temporal_annotation_txt(
                txt_path, vid_fname, duration_sec, annotation_fps=30.0
            )
            ...
            if gt_anno:
                ...
                print(f"  GT: '{vid_fname}' not found in {os.path.basename(txt_path)}, GT disabled")
        elif csv_path:
...
def parse_args():
    ...
    parser.add_argument("--gt-txt", type=str, default=r"C:\Users\USER\Desktop\ExploreVAD\annotation\Temporal_Anomaly_Annotation_For_Testing_Videos\Txt_formate\Temporal_Anomaly_Annotation.txt",
                        help="Path to Temporal_Anomaly_Annotation.txt "
                             "(frame-based, 30fps). Used for single video or video-dir. "
                             "Overrides --gt-dir when provided.")
...
def main():
    ...
    for video_path in video_files:
        ...
        if args.gt_txt:
            # Frame-based Temporal_Anomaly_Annotation.txt takes priority
            gt_timestamps_info = {
                'txt_path': args.gt_txt,
