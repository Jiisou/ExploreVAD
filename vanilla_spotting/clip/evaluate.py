#!/usr/bin/env python3
"""
Evaluation script for CLIP-based Anomaly Action Spotting Model.
...
def evaluate_model(
    ...
    Returns:
        Dictionary with labels, probs, and preds arrays.
    """
    model.eval()
...
def plot_roc_curve(labels: np.ndarray, probs: np.ndarray, save_path: str):
    ...
    plt.title('ROC Curve - CLIP Anomaly Action Spotting', fontsize=14)
...
def plot_precision_recall_curve(labels: np.ndarray, probs: np.ndarray, save_path: str):
    ...
    plt.title('Precision-Recall Curve - CLIP Anomaly Action Spotting', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
def plot_confusion_matrix(cm: np.ndarray, save_path: str):
    ...
    ax.set_title('Confusion Matrix - CLIP Anomaly Action Spotting', fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
def plot_video_timeline(video_scores: Dict, video_path: str, save_dir: str = "./plots"):
    ...
    plt.title(f'Anomaly Timeline: {os.path.basename(video_path)}', fontsize=12)
...
def evaluate(
    ...
    dataset = UCFCrimeSpottingDataset(
        ...
        transform=transform,
        overlap_ratio=data_cfg.infer_overlap_ratio,
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate CLIP-based Anomaly Action Spotting Model"
    )
